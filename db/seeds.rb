...
case ENV['SECTION']  #seedを実行する際にSECTION=○という引数に章番号を指定することで登録するデータを変えられるようにします。
  when "2"
    comics = [
  ...
  when "3"
    comics = [
      { title: 'ワンピース ONE PIECE' , genre: '少年漫画　海賊　冒険ファンタジー　バトル' , serialization_started_year: 1997, serialization_end_year: nil, publisher: '週間少年ジャンプ' },
      ]
  ...
  when "4-5"
    comics = [
      { title: 'ワンピース ONE PIECE', genre: '少年漫画　海賊　冒険ファンタジー　バトル', serialization_started_year: 1997, serialization_end_year: nil, publisher: '週間少年ジャンプ' },
      { title: 'ゴルゴ13', genre: 'ハードボイルド　漫画', serialization_started_year: 1968, serialization_end_year: nil, publisher: 'ビックコミック'},
      ]

    questions = [
      { content: '連載終了していますか？', algorithm: 'serialization_end', eval_value: nil },
      { content: '連載は週間少年ジャンプ?', algorithm: 'publisher_match', eval_value: '週間少年ジャンプ' },
      ]
